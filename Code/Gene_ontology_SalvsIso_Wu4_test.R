...
vfit <- lmFit(v, design)
vfit <- contrasts.fit(vfit, contrasts = "Conti") #TODO
...
png('./Figures_edgeR/voom_Plot.png',width = 40, height = 20, units = "cm", res=90)
plotSA(efit)
dev.off()
...
glMDPlot(tfit, anno = z$counts, coef=1, status=dt, main=colnames(tfit)[1],
         ...
         path = "/Users/jonas/Desktop/Analysis-master/Figures_edgeR", folder = "html",
         html = "glMDPlot", launch=TRUE)
