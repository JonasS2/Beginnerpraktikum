...
v <- voom(z2, design2, plot=TRUE) 


vfit <- lmFit(v, design2)
vfit <- contrasts.fit(vfit, contrasts = Conti)

efit <- eBayes(vfit)
plotSA(efit)

## /Users/jonas/Desktop/Analysis-master
png('./Figures_edgeR/voom_Plot_KOvsCo.png',width = 40, height = 20, units = "cm", res=90)
plotSA(efit)
dev.off()
...
glMDPlot(tfit, anno = z$counts, coef=1, status=dt, main=colnames(tfit)[1],
         ...
         html = "glMDPlot_KOvsCO", launch=FALSE)
